...
def convert_file(source, source_extension, converter, output_dir, output_extension):
    ...
    if source_extension.lower() != source_extension.lower():
        raise Exception(
            f" [!] You selected a wrong file type. Please select a(n) {source_extension.upper()} file.")
    output_file = os.path.basename(re.sub(
        f".{source_extension}", f".{output_extension}", source, flags=re.IGNORECASE))
    output_filepath = output_dir + "/" + output_file
...
def md_html(source, output_dir):
    ...
    if os.path.isfile(source):
        convert_file(source, "md", markdown_str_to_html_str,
                     output_dir, "html")
    elif os.path.isdir(source):
        convert_folder(source, "md", markdown_str_to_html_str,
                       output_dir, "html")
    else:
...
def md_dita(source, output_dir):
    ...
    if os.path.isfile(source):
        convert_file(source, "md", markdown_str_to_dita_str,
                     output_dir, "dita")
    elif os.path.isdir(source):
        convert_folder(source, "md", markdown_str_to_dita_str,
                       output_dir, "dita")
    else:
        raise Exception(f" [!] {source} does not exist.")
...
def html_dita(source, output_dir):
    ...
    if os.path.isfile(source):
        ...
        convert_folder(source, "html", html_str_to_dita_str,
                       output_dir, "dita")
    else:
        raise Exception(f" [!] {source} does not exist.")
...
def html_md(source, output_dir):
    """Convert Markdown to DITA."""
    if os.path.isfile(source):
        convert_file(source, "html", html_str_to_markdown_str,
                     output_dir, "md")
    elif os.path.isdir(source):
        convert_folder(source, "html", html_str_to_markdown_str,
                       output_dir, "md")
    else:
        raise Exception(f" [!] {source} does not exist.")
